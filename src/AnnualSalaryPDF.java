...
public final class AnnualSalaryPDF {
    ...
    private static double daysWorked(Employee person, int start, int index) { int month = index + 4; YearMonth period = month <= 12 ? YearMonth.of(start, month) : YearMonth.of(start + 1, month - 12); AttendanceDAO dao = new AttendanceDAO(); if (!dao.hasSavedAttendance(person.getId(), period.toString())) return 0; AttendanceRecord record = dao.load(person.getId(), period.toString()); AttendanceSettings settings = new AttendanceSettingsDAO().load(); return Money.round(Math.max(0, dao.workingDays(period.toString()) - record.absentDays - Math.max(0, record.paidLeaveDays - settings.casualLeaveLimit) - Math.max(0, record.unpaidLeaveDays - settings.earnedLeaveLimit))); }
