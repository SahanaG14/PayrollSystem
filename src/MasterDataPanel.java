...
public class MasterDataPanel extends JPanel {
    ...
    private static void normalizeImportDates(java.util.List<String> row){for(int column:new int[]{3,6,7})if(column<row.size()&&!cell(row,column).isEmpty())row.set(column,normalizeImportedDate(cell(row,column)));if(row.size()>10)row.set(10,cell(row,10).toUpperCase(java.util.Locale.ROOT));}
    ...
    public void refreshTable() {
        ...
        for (int i = 0; i < rows.length; i++) { Employee e = employees.get(i); rows[i] = new Object[]{e.getId(),e.getName(),e.getGender(),e.getDob(),e.getDepartment(),e.getDesignation(),e.getJoiningDate(),e.getEmploymentEndDate(),e.getStatus(),e.getSalaryStructure(),new FileCell("",e.getPhotoPath(),true),new FileCell(dao.panForDisplay(e),documentPart(e, 0),false),new FileCell(displayAadhaar(e),documentPart(e, 1),false),e.getUanNumber(),e.getEsicIpNumber(),e.getBankName(),e.getAccountNumber(),e.getIfsc(),e.getEmail(),e.getPhone(),e}; }
        table.setModel(new DefaultTableModel(rows, COLUMNS) { public boolean isCellEditable(int row, int column) { return column == 10 || column == 11 || column == 12 || column == 20; } }); UIStyleUtility.applyProfessionalTableStyle(table); table.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        for (int i = 0; i < COLUMNS.length; i++) table.getColumnModel().getColumn(i).setPreferredWidth(i == 1 || i == 18 ? 230 : (i == 20 ? 110 : 180));
        FileCellRenderer fileRenderer=new FileCellRenderer();FileCellEditor fileEditor=new FileCellEditor();for(int column:new int[]{10,11,12}){table.getColumnModel().getColumn(column).setCellRenderer(fileRenderer);table.getColumnModel().getColumn(column).setCellEditor(fileEditor);}table.getColumnModel().getColumn(20).setCellRenderer(new EditRenderer()); table.getColumnModel().getColumn(20).setCellEditor(new EditEditor());
    }
    ...
    private String displayAadhaar(Employee employee) { String value=employee==null?"":employee.getAadhar();return value!=null&&value.matches("\\d{12}")?value:""; }
    private void openSavedFile(String path){try{File file=new File(path);if(!file.isFile())throw new IllegalArgumentException();Desktop.getDesktop().open(file);}catch(Exception ignored){JOptionPane.showMessageDialog(this,"The saved file is not available.");}}
    private void showInFolder(String path){try{File file=new File(path);if(!file.isFile())throw new IllegalArgumentException();new ProcessBuilder("explorer.exe","/select,",file.getCanonicalPath()).start();}catch(Exception ignored){JOptionPane.showMessageDialog(this,"The saved file is not available.");}}
    private static final class FileCell { final String number,path;final boolean photo; FileCell(String number,String path,boolean photo){this.number=number==null?"":number;this.path=path==null?"":path;this.photo=photo;} boolean hasFile(){return !path.isBlank();} }
    private final class FileCellRenderer extends JPanel implements javax.swing.table.TableCellRenderer { private final JLabel number=new JLabel();private final JButton view=new JButton("View"),folder=new JButton("Show in Folder");FileCellRenderer(){setLayout(new FlowLayout(FlowLayout.CENTER,5,0));setOpaque(true);setBorder(new javax.swing.border.MatteBorder(0,0,1,1,UIStyleUtility.GRID));add(number);add(view);add(folder);}private void show(FileCell cell,Color background){number.setText(cell.number);number.setVisible(!cell.number.isBlank());view.setVisible(cell.hasFile());folder.setVisible(cell.hasFile()&&!cell.photo);setBackground(background);}public Component getTableCellRendererComponent(JTable t,Object value,boolean selected,boolean focused,int row,int column){show((FileCell)value,selected?t.getSelectionBackground():(row%2==0?Color.WHITE:UIStyleUtility.ZEBRA));return this;}}
    private final class FileCellEditor extends AbstractCellEditor implements javax.swing.table.TableCellEditor { private final JPanel panel=new JPanel(new FlowLayout(FlowLayout.CENTER,5,0));private final JLabel number=new JLabel();private final JButton view=new JButton("View"),folder=new JButton("Show in Folder");private FileCell cell;FileCellEditor(){panel.add(number);panel.add(view);panel.add(folder);view.addActionListener(e->{FileCell value=cell;fireEditingStopped();openSavedFile(value.path);});folder.addActionListener(e->{FileCell value=cell;fireEditingStopped();showInFolder(value.path);});}public Component getTableCellEditorComponent(JTable t,Object value,boolean selected,int row,int column){cell=(FileCell)value;number.setText(cell.number);number.setVisible(!cell.number.isBlank());view.setVisible(cell.hasFile());folder.setVisible(cell.hasFile()&&!cell.photo);panel.setBackground(t.getSelectionBackground());return panel;}public Object getCellEditorValue(){return cell;}}
