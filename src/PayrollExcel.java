...
public final class PayrollExcel {
  ...
  public static Sheet read(File f)throws Exception {if(f==null||!f.isFile()||f.length()>25L*1024*1024)throw new Exception("Import files must be smaller than 25 MB.");String n=f.getName().toLowerCase();if(n.endsWith(".csv"))return csv(f);if(n.endsWith(".xls"))throw new Exception("Legacy .xls files are not supported. Please save the file as .xlsx.");try(ZipFile z=new ZipFile(f)){ZipEntry e=z.getEntry("xl/worksheets/sheet1.xml");if(e==null||e.getSize()>20L*1024*1024)throw new Exception("Worksheet is missing or too large.");Document d=parse(z.getInputStream(e));List<String> shared=new ArrayList<>();ZipEntry ss=z.getEntry("xl/sharedStrings.xml");if(ss!=null){if(ss.getSize()>20L*1024*1024)throw new Exception("Shared strings are too large.");NodeList ts=parse(z.getInputStream(ss)).getElementsByTagNameNS("*","t");if(ts.getLength()>100000)throw new Exception("Too many spreadsheet cells.");for(int i=0;i<ts.getLength();i++)shared.add(ts.item(i).getTextContent());}List<List<String>> out=new ArrayList<>();NodeList rr=d.getElementsByTagNameNS("*","row");if(rr.getLength()>10000)throw new Exception("Too many spreadsheet rows.");for(int i=0;i<rr.getLength();i++){Element row=(Element)rr.item(i);List<String> values=new ArrayList<>();NodeList cc=row.getElementsByTagNameNS("*","c");if(cc.getLength()>200)throw new Exception("Too many spreadsheet columns.");for(int j=0;j<cc.getLength();j++){Element c=(Element)cc.item(j);int idx=colIndex(c.getAttribute("r"));if(idx>200)throw new Exception("Spreadsheet column is too large.");while(values.size()<idx)values.add("");String type=c.getAttribute("t"),v="";NodeList is=c.getElementsByTagNameNS("*","t");if(is.getLength()>0)v=is.item(0).getTextContent();else {NodeList vs=c.getElementsByTagNameNS("*","v");if(vs.getLength()>0)v=vs.item(0).getTextContent();if("s".equals(type)&&!v.isEmpty())v=shared.get(Integer.parseInt(v));}values.add(v);}out.add(values);}return new Sheet(out);}}
