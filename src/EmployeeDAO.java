...
public class EmployeeDAO {
    ...
    private void bind(PreparedStatement p, Employee e, boolean update) throws SQLException {
        String employeeId=e.getId()==null?"":e.getId().trim();if(employeeId.isEmpty())throw new SQLException("Employee ID is required");e.setId(employeeId);String phone=e.getPhone()==null?"":e.getPhone().replaceAll("\\D","");if(phone.length()!=10)throw new SQLException("Phone number must contain exactly 10 digits.");e.setPhone(phone);String pan=e.getPan()==null?"":e.getPan().trim().toUpperCase(Locale.ROOT);if(!pan.isEmpty()&&!validPan(pan))throw new SQLException("PAN must use the format AAAAA9999A.");e.setPan(pan);String aadhaar=e.getAadhar()==null?"":e.getAadhar().replaceAll("\\D","");if(!aadhaar.isEmpty()&&!aadhaar.matches("\\d{12}"))throw new SQLException("Aadhaar number must contain exactly 12 digits.");e.setAadhar(aadhaar);String account=e.getAccountNumber()==null?"":e.getAccountNumber().replaceAll("\\s","");if(!account.matches("\\d{9,18}"))throw new SQLException("Bank Account Number must contain between 9 and 18 digits.");e.setAccountNumber(account);String[] documents=(e.getDocumentPaths()==null?"":e.getDocumentPaths()).split("\\|",-1);int i=1; if (!update) p.setString(i++,employeeId); p.setString(i++,e.getName()); p.setString(i++,e.getGender()); p.setString(i++,e.getDob()); p.setString(i++,e.getDepartment()); p.setString(i++,e.getDesignation()); p.setString(i++,e.getJoiningDate()); p.setString(i++,e.getStatus()); p.setString(i++,e.getEmploymentEndDate()); p.setString(i++,protect(e.getEmail())); p.setString(i++,phone); p.setString(i++,pan); p.setString(i++,aadhaar);p.setString(i++,"");p.setString(i++,"");p.setString(i++,""); p.setString(i++,protect(e.getEsicIpNumber())); p.setString(i++,protect(e.getUanNumber())); p.setString(i++,protect(e.getBankName())); p.setString(i++,e.getAccountNumber()); p.setString(i++,protect(e.getIfsc())); p.setString(i++,e.getSalaryStructure()==null?"Wages":e.getSalaryStructure()); p.setString(i++,e.getPhotoPath()); p.setString(i++,documents.length>0?documents[0]:""); p.setString(i++,documents.length>1?documents[1]:"");p.setBoolean(i++,e.isRestrictEpfCeiling()); if(update)p.setString(i,employeeId);
    }
    ...
    private Employee read(ResultSet r)throws SQLException { Employee e=new Employee();e.setId(get(r,"employee_id"));e.setName(get(r,"employee_name"));e.setGender(get(r,"gender"));e.setDob(get(r,"dob"));e.setDepartment(get(r,"department"));e.setDesignation(get(r,"designation"));e.setJoiningDate(get(r,"joining_date"));e.setStatus(get(r,"status"));e.setEmploymentEndDate(get(r,"leaving_date"));e.setEmail(unprotect(get(r,"email")));e.setPhone(readPhone(r));e.setPan(readPan(r));e.setAadhar(readAadhaar(r));e.setEsicIpNumber(unprotect(get(r,"esic")));e.setUanNumber(unprotect(get(r,"uan")));e.setBankName(unprotect(get(r,"bank_name")));e.setAccountNumber(unprotect(get(r,"bank_acc")));e.setIfsc(unprotect(get(r,"ifsc")));e.setPhotoPath(get(r,"photo_path"));e.setDocumentPaths(get(r,"pan_doc_path")+"|"+get(r,"aadhaar_doc_path"));e.setRestrictEpfCeiling("1".equals(get(r,"epf_ceiling")));String structure=get(r,"salary_structure");e.setSalaryStructure("Salary".equalsIgnoreCase(structure)||"Wages".equalsIgnoreCase(structure)?structure:"Wages");return e; }
    ...
    private String readPhone(ResultSet r)throws SQLException{String value=get(r,"phone");if(digits(value,10)&&!"0000000000".equals(value))return value;String legacy=unprotect(get(r,"phone_ciphertext"));return digits(legacy,10)?legacy:"";}
    private String readPan(ResultSet r)throws SQLException{String value=get(r,"pan").trim().toUpperCase(Locale.ROOT);if(validPan(value))return value;String legacy=unprotect(get(r,"pan_ciphertext"));return validPan(legacy)?legacy.trim().toUpperCase(Locale.ROOT):"";}
    private String readAadhaar(ResultSet r)throws SQLException{String value=get(r,"aadhaar");if(digits(value,12)&&!"000000000000".equals(value))return value;String legacy=unprotect(get(r,"aadhaar_ciphertext"));return digits(legacy,12)?legacy:"";}
    private boolean digits(String value,int length){if(value==null||value.length()!=length)return false;for(int i=0;i<value.length();i++)if(value.charAt(i)<'0'||value.charAt(i)>'9')return false;return true;}
