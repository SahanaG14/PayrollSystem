...
public class EmployeeDAO {
    ...
    private void bind(PreparedStatement p, Employee e, boolean update) throws SQLException {
        String employeeId=e.getId()==null?"":e.getId().trim();if(employeeId.isEmpty())throw new SQLException("Employee ID is required");e.setId(employeeId);String phone=e.getPhone()==null?"":e.getPhone().replaceAll("\\D","");if(phone.length()!=10)throw new SQLException("Phone number must contain exactly 10 digits.");e.setPhone(phone);String[] documents=(e.getDocumentPaths()==null?"":e.getDocumentPaths()).split("\\|",-1);int i=1; if (!update) p.setString(i++,employeeId); p.setString(i++,e.getName()); p.setString(i++,e.getGender()); p.setString(i++,e.getDob()); p.setString(i++,e.getDepartment()); p.setString(i++,e.getDesignation()); p.setString(i++,e.getJoiningDate()); p.setString(i++,e.getStatus()); p.setString(i++,e.getEmploymentEndDate()); p.setString(i++,protect(e.getEmail())); p.setString(i++,protect(phone)); p.setString(i++,protect(e.getPan())); p.setString(i++,protect(e.getAadhar())); p.setString(i++,protect(e.getEsicIpNumber())); p.setString(i++,protect(e.getUanNumber())); p.setString(i++,protect(e.getBankName())); p.setString(i++,protect(e.getAccountNumber())); p.setString(i++,protect(e.getIfsc())); p.setString(i++,e.getSalaryStructure()==null?"Wages":e.getSalaryStructure()); p.setString(i++,e.getPhotoPath()); p.setString(i++,documents.length>0?documents[0]:""); p.setString(i++,documents.length>1?documents[1]:"");p.setBoolean(i++,e.isRestrictEpfCeiling()); if(update)p.setString(i,employeeId);
    }
    ...
    private Employee read(ResultSet r)throws SQLException { Employee e=new Employee();e.setId(get(r,"employee_id"));e.setName(get(r,"employee_name"));e.setGender(get(r,"gender"));e.setDob(get(r,"dob"));e.setDepartment(get(r,"department"));e.setDesignation(get(r,"designation"));e.setJoiningDate(get(r,"joining_date"));e.setStatus(get(r,"status"));e.setEmploymentEndDate(get(r,"leaving_date"));e.setEmail(unprotect(get(r,"email")));e.setPhone(unprotect(get(r,"phone")));e.setPan(unprotect(get(r,"pan")));e.setAadhar(unprotect(get(r,"aadhaar")));e.setEsicIpNumber(unprotect(get(r,"esic")));e.setUanNumber(unprotect(get(r,"uan")));e.setBankName(unprotect(get(r,"bank_name")));e.setAccountNumber(unprotect(get(r,"bank_acc")));e.setIfsc(unprotect(get(r,"ifsc")));e.setPhotoPath(get(r,"photo_path"));e.setDocumentPaths(get(r,"pan_doc_path")+"|"+get(r,"aadhaar_doc_path"));e.setRestrictEpfCeiling("1".equals(get(r,"epf_ceiling")));String structure=get(r,"salary_structure");e.setSalaryStructure("Salary".equalsIgnoreCase(structure)||"Wages".equalsIgnoreCase(structure)?structure:"Wages");return e; }
