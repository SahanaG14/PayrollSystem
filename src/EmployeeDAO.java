...
public class EmployeeDAO {
    ...
    private static final File CACHE=new File(System.getProperty("user.home"),".payroll-employees-cache.bin");
    ...
    private void ensureTable() {
        try (Connection c=DBConnection.getConnection(); Statement s=c==null?null:c.createStatement()) {
            if (s != null) { ensureColumn(s,"is_deleted INTEGER NOT NULL DEFAULT 0"); ensureColumn(s,"deleted_at TEXT"); ensureColumn(s,"epf_ceiling INTEGER NOT NULL DEFAULT 0"); }
    ...
    public boolean existsMasterDataId(String id) {
        ...
        return containsEmployeeId(MEMORY,employeeId)||containsEmployeeId(ROSTER,employeeId)||containsEmployeeId(readCache(),employeeId);
    }
    ...
    private void upsert(Employee e, boolean update) throws SQLException {
        ...
        String cols="employee_id,employee_name,gender,dob,department,designation,joining_date,status,leaving_date,email,phone,pan,aadhaar,esic,uan,bank_name,bank_acc,ifsc,salary_structure,photo_path,pan_doc_path,aadhaar_doc_path,epf_ceiling";
        String sql=update ? "UPDATE employee_master_data SET employee_name=?,gender=?,dob=?,department=?,designation=?,joining_date=?,status=?,leaving_date=?,email=?,phone=?,pan=?,aadhaar=?,esic=?,uan=?,bank_name=?,bank_acc=?,ifsc=?,salary_structure=?,photo_path=?,pan_doc_path=?,aadhaar_doc_path=?,epf_ceiling=? WHERE employee_id=? AND is_deleted=0" : "INSERT INTO employee_master_data ("+cols+") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    private void bind(PreparedStatement p, Employee e, boolean update) throws SQLException {
        String employeeId=e.getId()==null?"":e.getId().trim();if(employeeId.isEmpty())throw new SQLException("Employee ID is required");e.setId(employeeId);String phone=e.getPhone()==null?"":e.getPhone().replaceAll("\\D","");if(phone.length()!=10)throw new SQLException("Phone number must contain exactly 10 digits.");e.setPhone(phone);String[] documents=(e.getDocumentPaths()==null?"":e.getDocumentPaths()).split("\\|",-1);int i=1; if (!update) p.setString(i++,employeeId); p.setString(i++,e.getName()); p.setString(i++,e.getGender()); p.setString(i++,e.getDob()); p.setString(i++,e.getDepartment()); p.setString(i++,e.getDesignation()); p.setString(i++,e.getJoiningDate()); p.setString(i++,e.getStatus()); p.setString(i++,e.getEmploymentEndDate()); p.setString(i++,e.getEmail()); p.setString(i++,phone); p.setString(i++,e.getPan()); p.setString(i++,e.getAadhar()); p.setString(i++,e.getEsicIpNumber()); p.setString(i++,e.getUanNumber()); p.setString(i++,e.getBankName()); p.setString(i++,e.getAccountNumber()); p.setString(i++,e.getIfsc()); p.setString(i++,e.getSalaryStructure()==null?"Wages":e.getSalaryStructure()); p.setString(i++,e.getPhotoPath()); p.setString(i++,documents.length>0?documents[0]:""); p.setString(i++,documents.length>1?documents[1]:"");p.setBoolean(i++,e.isRestrictEpfCeiling()); if(update)p.setString(i,employeeId);
    }
    ...
    @SuppressWarnings("unchecked") private List<Employee> readCache(){if(!CACHE.isFile())return new ArrayList<>();try(ObjectInputStream in=new ObjectInputStream(new FileInputStream(CACHE))){return new ArrayList<>((List<Employee>)in.readObject());}catch(Exception ignored){return new ArrayList<>();}}
    private void cache(List<Employee> employees){if(employees==null)return;try(ObjectOutputStream out=new ObjectOutputStream(new FileOutputStream(CACHE))){out.writeObject(new ArrayList<>(employees));}catch(Exception ignored){}}
    private Employee read(ResultSet r)throws SQLException { Employee e=new Employee();e.setId(get(r,"employee_id"));e.setName(get(r,"employee_name"));e.setGender(get(r,"gender"));e.setDob(get(r,"dob"));e.setDepartment(get(r,"department"));e.setDesignation(get(r,"designation"));e.setJoiningDate(get(r,"joining_date"));e.setStatus(get(r,"status"));e.setEmploymentEndDate(get(r,"leaving_date"));e.setEmail(get(r,"email"));e.setPhone(get(r,"phone"));e.setPan(get(r,"pan"));e.setAadhar(get(r,"aadhaar"));e.setEsicIpNumber(get(r,"esic"));e.setUanNumber(get(r,"uan"));e.setBankName(get(r,"bank_name"));e.setAccountNumber(get(r,"bank_acc"));e.setIfsc(get(r,"ifsc"));e.setPhotoPath(get(r,"photo_path"));e.setDocumentPaths(get(r,"pan_doc_path")+"|"+get(r,"aadhaar_doc_path"));e.setRestrictEpfCeiling("1".equals(get(r,"epf_ceiling")));String structure=get(r,"salary_structure");e.setSalaryStructure("Salary".equalsIgnoreCase(structure)||"Wages".equalsIgnoreCase(structure)?structure:"Wages");return e; }
